...
@class NIMNetCallVideoDevice;
@class NIMNetCallMacAudioDevice;
@class NIMNetCallSocksParam;
@class NIMNetCallMP4RecordOption;
@class NIMNetCallCustomVideoParam;
@class NIMNetCallRenderMode;
@class NIMMessage;
@class NIMNetCallRecordsSearchOption;
 ...
 *  @param error   查询状态结果
 */
typedef void(^NIMNetCallSearchRecordsHandler)(NSArray<NIMMessage *> * _Nullable records, NSError * _Nullable error);
 ...
 *  开始MP4文件录制, 录制通话过程中自己的音视频内容到MP4文件
 *
 *  @param filePathURL 录制文件路径, SDK不负责创建目录, 请确保文件路径的合法性,
 *                      也可以传入nil, 由SDK自己选择文件路径
 *  @param option       录制文件视频参数设置, 可以不指定, 由SDK自己选择合适的参数值
 *
 *  @param userId       录制用户对象的id
 *
 *  @return 是否允许开始录制
 ...
 *  @discussion 只有通话连接建立以后才允许开始录制
 */
- (BOOL)startRecording:(nullable NSURL *)filePathURL
          option:(NIMNetCallMP4RecordOption *)option
                   uid:(NSString *)userId;
...
/**
 ...
 @param socksParam 代理配置参数
 
 @discussion 不可与参数配置方式同时使用, 建议使用接口方式设置代理参数
 */
